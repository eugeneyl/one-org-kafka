...
app.post('/api/createProduct/', async function(req, res) {
    try {
        ...
        await gateway.connect(ccpPath, { wallet, identity: user, discovery: { enabled: true, asLocalhost: false },eventHandlerOptions:{strategy:null}});
        const network = await gateway.getNetwork('mychannel');
        const contract = network.getContract('magento_product');
        await contract.submitTransaction('createProduct', req.body.sku);
...
app.post('/api/createOrder/', async function(req, res) {
    try {
        ...
        await contract.submitTransaction('createOrder', req.body.entity_id);
        console.log('Transaction has been submitted');
        res.send('Transaction has been submitted');
        await gateway.disconnect();
    } catch (error) {
...
app.get('/api/queryOrder/', async function(req, res) {
    try {
        ...
        const network = await gateway.getNetwork('mychannel');
        const contract = network.getContract('magento_order');
        const result = await contract.evaluateTransaction('queryOrder', req.body.entity_id);
        console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
        res.status(200).json({response: result.toString()});
    } catch (error) {
        console.error(`Failed to evaluate transaction: ${error}`);
...
app.put('/api/editOrder/', async function(req, res) {
    try {
        ...
        const network = await gateway.getNetwork('mychannel');
        const contract = network.getContract('magento_order');
        await contract.submitTransaction('editOrder', req.body.entity_id);
        console.log('Transaction has been submitted');
        res.send('Transaction has been submitted');
    } catch (error){
        console.error(`Failed to evaluate transaction: ${error}`);
...
app.delete('/api/deleteOrder/', async function(req, res) {
    try {
        ...
        const network = await gateway.getNetwork('mychannel');
        const contract = network.getContract('magento_order');
        await contract.submitTransaction('deleteOrder', req.body.entity_id);
        console.log('Transaction has been submitted');
        res.send('Transaction has been submitted');
    } catch (error) {
        console.error(`Failed to evaluate transaction: ${error}`);
